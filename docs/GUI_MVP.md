...
- PGN-блок для импорта и экспорта партии через текстовое поле.
- UCI-лог.
...
- Сохранение и открытие PGN через файловый диалог.
- Контроль времени.
...
3. PGN-список ходов в SAN рядом с UCI-историей.
4. Сохранение и открытие PGN из файла.
5. Отдельная панель legal moves.
6. Кнопка `perft` для текущей позиции.
7. Вывод nodes/depth из UCI `info` в отдельные поля, а не только в сырой лог.
...
Пока это текстовый MVP без файлового диалога. Это сделано специально: сначала нужен проверяемый формат данных, потом удобная оболочка.
